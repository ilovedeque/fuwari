...
* $1 \le n \le 10^5$.
* $0 \le k \le 10^{12}$.
* $0 \le a_i \le 10^6 \ \forall \ i \in \{1, 2, ..., n\}$.
...
Chúng ta chỉ cần duyệt qua tất cả dãy con $(i, j)$ $(1 \le i \le j \le n)$ rồi kiểm tra xem $\sum_{x=i}^{j} {(a_x)} \le k$ không. Nếu có thì $ans \leftarrow \max(ans, j - i + 1)$.
...
* $1 \le n \le 10^5$.
* $1 \le q \le 10^5$.
* $n \cdot (q + 1) \le 5 \cdot 10^5$.

| Subtask | $\%$ số điểm |              Ràng buộc bổ sung              |
|:-------:|:------------:|:-------------------------------------------:|
|   $1$   |     $40$     |                $n \le 10^2$                 |
|   $2$   |     $30$     | Xâu $s$ chỉ chứa $2$ chữ số '$0$' và '$1$'. |
|   $3$   |     $30$     |         Không có ràng buộc gì thêm.         |
...
Sau khi duyệt xong, với mỗi chữ số $d$ từ $0$ đến $9$, chúng ta lấy $P \leftarrow P+min(cnts[d], cntt[d])$.
...
* $1 \le n \le 100$.
* $1 \le S \le 10^9$.
* $1 \le a_i \le 100 \ \forall \ i \in \{1, 2, ..., n\}$.
