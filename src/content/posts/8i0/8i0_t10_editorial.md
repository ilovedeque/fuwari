...
Chúng ta có thể duyệt hết các chỉ sổ $i$ thoả mãn điều kiện $1 \le i \le n - m + 1$ rồi tính $res=\sum_{k=i}^{i+m-1} {(a_k)}$ rồi cập nhật $ans \leftarrow \max(ans, \ res)$.
...
**Độ phức tạp:** $O(T \cdot (|a+b| \log |a+b| + |c| \log |c|))$.
